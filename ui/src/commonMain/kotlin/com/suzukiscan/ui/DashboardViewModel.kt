...
import com.suzukiscan.core.field.niceScaleMin
...
class DashboardViewModel(
    ...
    fun persistCurrentConnectionLog() {
        ...
        if (contents != "No adapter traffic logged yet.") {
            ...
            exportSink(listOf(ExportFile("suzuki-scan-connection-log-$id.txt", contents)))
            currentIoLog = null
            currentIoLogId = null
        }
    }
    ...
    private fun finishCurrentRun() {
        ...
        if (id != null && readings.isNotEmpty() && _completedRuns.value.none { it.id == id }) {
            ...
            exportSink(listOf(ExportFile("suzuki-scan-run-$id.csv", recorder.toCsv(
