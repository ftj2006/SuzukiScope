...
object ExportStore {
    private fun dayFolder(name: String): File = File(
        File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOCUMENTS), "pj-viewer"),
        "$name/${LocalDate.now()}",
    ).apply { mkdirs() }

    fun save(context: Context, files: List<com.suzukiscan.ui.DashboardViewModel.ExportFile>) {
        files.forEach { file ->
            val folderName = if (file.name.endsWith(".csv")) "Logs" else "Debug"
            val relativePath = "${Environment.DIRECTORY_DOCUMENTS}/pj-viewer/$folderName/${LocalDate.now()}/"
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.Q) {
                ...
                File(context.getExternalFilesDir(Environment.DIRECTORY_DOCUMENTS), "pj-viewer/$folderName/${LocalDate.now()}")
                    .apply { mkdirs() }
                    .resolve(file.name)
    ...
    fun saveDebugEntry(context: Context, entry: String) {
        save(context, listOf(com.suzukiscan.ui.DashboardViewModel.ExportFile(
